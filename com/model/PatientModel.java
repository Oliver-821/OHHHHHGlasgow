...
public class PatientModel {
	...
	public boolean addPatient(Patient p) {
		...
		try {
			...
			String sql = "insert into patient(room,name,sex,entertime,doctor,section,status,waiting) values (?,?,?,getdate(),?,?,?,?)";
	...
	public Patient queryOne(int id) {
		...
		String sql = "select id,name,sex,doctor,section,entertime,room,status from patient where id="+id;
		PreparedStatement pst = null;
		Connection conn = null;
		ResultSet rs = null;
		try {
	...
	public boolean uptPatient(Patient p, int id) {
		...
		try {
			conn = DBHelp.getConnection();
			String sql = "update patient set name=?,sex=?,doctor=?,section=?,room=?waiting=?,status=? where id="+id;
			pst = conn.prepareStatement(sql);
